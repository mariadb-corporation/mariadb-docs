...
When writing a regular expression (regex) type parameter to a MaxScale configuration file, the pattern string should be enclosed in slashes e.g. `^select` -> `match=/^select/`. This clarifies where the pattern begins and ends, even if it includes whitespace. Without slashes the configuration loader trims the pattern from the ends. The slashes are removed before compiling the pattern. For backwards compatibility, the slashes are not mandatory. Omitting them is, however, deprecated. The _binlogfilter_, _ccrfilter_, _namedserverfilter_, _qlafilter_, _regexfilter_, _tee_, _topfilter_, _avrorouter_, _kafkacdc_, and _schemarouter_ modules accept parameters in this type of regular expression form. Other modules may not handle the slashes correctly.
...
The trace files differ from the normal log file written by MaxScale in that they do not contain the local timestamp and instead contain a raw fractional UNIX timestamp. The format of the trace file is subject to change.
...
How many log entries are stored in the session specific trace log. This log is written to disk when a session ends abnormally and can be used for debugging purposes. The session trace log is written to the log in the following situations:
...
Verify the peer (client) TLS certificate for REST-API connections. This is the REST-API counterpart to the listener [ssl\_verify\_peer\_certificate](maxscale-configuration-guide.md#ssl_verify_peer_certificate) parameter.

When enabled, clients connecting to the REST API must present a certificate, and that certificate is verified against the CA configured with [admin\_ssl\_ca](maxscale-configuration-guide.md#admin_ssl_ca). If `admin_ssl_ca` is not set, verification is not performed.
...
Verify the peer host for REST-API TLS connections. This is the REST-API counterpart to the listener [ssl\_verify\_peer\_host](maxscale-configuration-guide.md#ssl_verify_peer_host) parameter.

When enabled, the client's hostname or IP address is verified against the certificate it presents; if they do not match, the connection is closed. If the client does not provide a certificate, host verification is skipped. To require client certificates, also enable [admin\_ssl\_verify\_peer\_certificate](maxscale-configuration-guide.md#admin_ssl_verify_peer_certificate).
...
The encryption key is only read on startup. This means that the tokens will be signed by the latest key version that is available on startup: rotating the encryption key in the key management system will not cause the JWTs to be signed with newer versions of the key.
...
List of comma separated HTTP methods to exclude from logging. MaxScale does not use `CONNECT` or `TRACE`.
...
This parameter was moved into the MaxScale core in MaxScale 6.0. The parameter can be configured for all routers that support the session command history. The `readwritesplit`, `schemarouter`, and `smartrouter` routers support it.
...
A number of routing modules are available, these are designed for a range of different needs.
