...
* Port backup features from Enterprise Server. [mariadb-backup]({server}/server-usage/backup-and-restore/mariadb-backup) now performs its work across all five [BACKUP STAGE]({server}/reference/sql-statements/administrative-sql-statements/backup-commands/backup-stage) phases instead of doing almost all of it while holding `BLOCK_COMMIT`, which substantially shortens the window in which commits are blocked, and obsoletes the [no-backup-locks]({server}/server-usage/backup-and-restore/mariadb-backup/mariadb-backup-options#no-backup-locks) and [rsync]({server}/server-usage/backup-and-restore/mariadb-backup/mariadb-backup-options#rsync) options ([MDEV-32932](https://jira.mariadb.org/browse/MDEV-32932))
