...
To re-gain a database level privileges, using a user with `SHOW CREATE ROUTINE` on `*.*` execute [GRANT ALL ON db.\* TO user]({server}/reference/sql-statements/account-management-sql-statements/grant), optionally WITH GRANT OPTION.
