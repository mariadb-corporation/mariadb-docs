...
To re-gain a database level privileges, using a user with `SHOW CREATE ROUTINE` on `*.*` execute [GRANT ALL ON db.\* TO user](https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/reference/sql-statements/account-management-sql-statements/grant), optionally WITH GRANT OPTION.
...
For a complete list of changes made in [MariaDB 11.3.1](11.3.1.md), with links to detailed information on each push, see the [changelog](../../changelogs/11.3/mariadb-11-3-1-changelog.md).
