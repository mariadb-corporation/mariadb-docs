...
To re-gain a database level privileges, using a user with `SHOW CREATE ROUTINE` on `*.*` execute [GRANT ALL ON db.\* TO user](https://app.gitbook.com/o/diTpXxF5WsbHqTReoBsS/s/SsmexDFPv2xG2OTyO5yV/reference/sql-statements/account-management-sql-statements/grant), optionally WITH GRANT OPTION.
