...
MariaDB is kept up to date with the latest MySQL release from the same branch.[MariaDB]({server}) 5.1.42 is based on [MySQL](https://www.mysql.com/) 5.1.42 and [XtraDB](https://www.percona.com/docs/wiki/percona-xtradb:start) 1.0.6-9.
...
We are working on setting up a [BuildBot]({server}/reference/product-development/server-development/tools/buildbot/about-buildbot) network which will enable
