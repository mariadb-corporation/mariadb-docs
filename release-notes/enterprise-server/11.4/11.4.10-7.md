...
* [CHECK TABLE]({server}/reference/sql-statements/table-statements/check-table) and [mariadb-check]({server}/clients-and-utilities/table-tools/mariadb-check) now support [SEQUENCE tables]({server}/server-usage/storage-engines/sequence-storage-engine). Previously, checking a sequence returned the note `The storage engine for the table doesn't support check` ([MDEV-22491](https://jira.mariadb.org/browse/MDEV-22491))
