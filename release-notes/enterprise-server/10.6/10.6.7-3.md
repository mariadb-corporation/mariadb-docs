...
MariaDB Enterprise Server 10.6.7-3 is a Stable (GA) maintenance release of [MariaDB Enterprise Server]({release-notes}/enterprise-server) 10.6. This release includes a variety of fixes.
...
* When a double-encapsulated CTE query calls a function which reads a table that has been aliased in the CTE query, the server incorrectly raises an error with the [ER\_NO\_SUCH\_TABLE]({server}/reference/error-codes/mariadb-error-codes-1100-to-1199/e1146) error code. ([MDEV-26825](https://jira.mariadb.org/browse/MDEV-26825))
* When a CTE is used in a subquery of a `DELETE` statement, the server incorrectly raises an error with the [ER\_NO\_DB\_ERROR]({server}/reference/error-codes/mariadb-error-codes-1000-to-1099/e1046) error code. ([MDEV-26470](https://jira.mariadb.org/browse/MDEV-26470))
