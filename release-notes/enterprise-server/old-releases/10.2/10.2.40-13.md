...
This thirteenth release of [MariaDB Enterprise Server]({release-notes}/enterprise-server) 10.2 is a maintenance release. This release includes a variety of fixes.
...
* MariaDB Enterprise Backup expects [--stream=mbstream]({server}/server-usage/backup-and-restore/mariadb-backup/mariadb-backup-options#stream) to stream a backup using the included `mbstream` utility. ([MDEV-15730](https://jira.mariadb.org/browse/MDEV-15730))
  * Prior to this release, MariaDB Enterprise Backup expected [--stream=mbstream]({server}/server-usage/backup-and-restore/mariadb-backup/mariadb-backup-options#stream), even though the `xbstream` utility had been renamed to `mbstream.`
