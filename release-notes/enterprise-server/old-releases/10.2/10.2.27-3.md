...
This third release of [MariaDB Enterprise Server]({release-notes}/enterprise-server) 10.2 is a maintenance release, including a variety of fixes.
...
* Corner case where [MariaDB Backup]({server}/server-usage/backup-and-restore/mariadb-backup) could fail because encrypted data files could not be copied if created shortly before the backup was started. ([MDEV-18128](https://jira.mariadb.org/browse/MDEV-18128))
* [MariaDB Backup]({server}/server-usage/backup-and-restore/mariadb-backup) could fail in the prepare face if the redo log is bigger than 512GB. ([MDEV-20060](https://jira.mariadb.org/browse/MDEV-20060))
