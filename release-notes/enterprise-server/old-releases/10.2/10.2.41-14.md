...
MariaDB Enterprise Server 10.2.41-14 is a maintenance release of [MariaDB Enterprise Server]({release-notes}/enterprise-server) 10.2. This release includes a variety of fixes.
...
* [MariaDB Enterprise Cluster]({galera}) can crash due to an incorrect conflict resolution on multi-master setup. ([MDEV-25114](https://jira.mariadb.org/browse/MDEV-25114))
...
* [MariaDB Enterprise Cluster]({galera}) joiner node incorrectly uses `localhost` for TLS certificate verification and fails to join cluster when [wsrep\_sst\_method=mariadb-backup]({galera}/reference/galera-cluster-system-variables#wsrep_sst_method) and `encrypt=3` are configured. ([MDEV-26360](https://jira.mariadb.org/browse/MDEV-26360))
