...
This second release of [MariaDB Enterprise Server]({release-notes}/enterprise-server) 10.2 is a maintenance release, addressing security vulnerabilities and including a variety of fixes.
...
* Removed [--rsync]({server}/server-usage/backup-and-restore/mariadb-backup/mariadb-backup-options#rsync) legacy option from enterprise build of MariaDB Backup
...
* Possible crash when [Galera]({galera}) and foreign key is used, if the referenced record is deleted at the same time ([MDEV-19660](https://jira.mariadb.org/browse/MDEV-19660))
* Recovery with [mariadb-backup]({server}/server-usage/backup-and-restore/mariadb-backup) crash related to [InnoDB](https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/server-usage/storage-engines/innodb) with custom [innodb\_data\_file\_path](https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/server-usage/storage-engines/myrocks/myrocks-system-variables#innodb_data_file_path) ([MDEV-19978](https://jira.mariadb.org/browse/MDEV-19978))
...
* Fix [Aria ER\_CRASHED\_ON\_USAGE]({server}/reference/error-codes/mariadb-error-codes-1100-to-1199/e1194) and Assertion ([MDEV-19595](https://jira.mariadb.org/browse/MDEV-19595))

## Interface Changes

* [mariadb-backup]({server}/server-usage/backup-and-restore/mariadb-backup) [--rsync option]({server}/server-usage/backup-and-restore/mariadb-backup/mariadb-backup-options#rsync) removed
