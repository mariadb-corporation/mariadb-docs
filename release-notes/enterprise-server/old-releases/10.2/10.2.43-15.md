...
MariaDB Enterprise Server 10.2.43-15 is a maintenance release of [MariaDB Enterprise Server]({release-notes}/enterprise-server) 10.2. This release includes a variety of fixes.
...
* When the [version](https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/server-management/variables-and-modes/server-system-variables#version) system variable is set, MariaDB Connector/C and the [mariadb]({server}/clients-and-utilities/mariadb-client/mariadb-command-line-client) client do not interpret MariaDB Server's extended metadata properly, which can cause results from some [SHOW ..](https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/reference/sql-statements/administrative-sql-statements/show) to be right-aligned. ([MDEV-27304](https://jira.mariadb.org/browse/MDEV-27304))
...
* When a CTE is used with a `UNION` in a [CREATE TABLE .. AS ..](https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/server-usage/tables/create-table) statement, the server incorrectly raises an error with the [ER\_NO\_DB\_ERROR]({server}/reference/error-codes/mariadb-error-codes-1000-to-1099/e1046) error code. ([MDEV-27086](https://jira.mariadb.org/browse/MDEV-27086))
...
* When [wsrep\_sst\_method=mariadb-backup]({galera}/reference/galera-cluster-system-variables#wsrep_sst_method) is set for MariaDB Enterprise Cluster, but MariaDB Enterprise Backup is not installed on the joiner node, the joiner node fails to start. ([MDEV-15209](https://jira.mariadb.org/browse/MDEV-15209))
