...
* For [MariaDB Enterprise Cluster]({galera}) (Galera Library 25.3.29):
...
* [MariaDB Enterprise Backup]({server}/server-usage/backup-and-restore/mariadb-enterprise-backup) could encounter a deadlock if a backup is taken from a replica with [slave-parallel-threads](https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/server-management/variables-and-modes/server-system-variables#slave_parallel_threads) is set to a value greater than `0` ([MDEV-21255](https://jira.mariadb.org/browse/MDEV-21255))
* [MariaDB Enterprise Backup]({server}/server-usage/backup-and-restore/mariadb-enterprise-backup) could crash when used in combination with `FLUSH TABLES` and `UNLOCK TABLES` (MENT-438)
...
* Dropping a partition with [wsrep\_OSU\_method=RSU](https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/server-management/variables-and-modes/server-system-variables#wsrep_OSU_method) and SESSION [sql\_log\_bin=0](https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/ha-and-performance/standard-replication/replication-and-binary-log-system-variables#sql_log_bin) caused the [MariaDB Enterprise Cluster]({galera}) (Galera) node to hang. ([MDEV-21189](https://jira.mariadb.org/browse/MDEV-21189))

### Can result in unexpected behavior

* Executing [TRUNCATE]({server}/reference/sql-statements/table-statements/truncate-table) or [OPTIMIZE]({server}/ha-and-performance/optimization-and-tuning/optimizing-tables/optimize-table) on [InnoDB](https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/server-usage/storage-engines/innodb) tables could lead to an unexpected `SQL Error (1118): Row size too large`, when [innodb\_strict\_mode=ON](https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/server-management/variables-and-modes/server-system-variables#innodb_strict_mode) (default). ([MDEV-21429](https://jira.mariadb.org/browse/MDEV-21429))
...
* Using the --use-memory option with [mariadb-backup]({server}/server-usage/backup-and-restore/mariadb-backup) would cause it to run out of memory. ([MDEV-20679](https://jira.mariadb.org/browse/MDEV-20679))
* For [MariaDB Enterprise Cluster]({galera}) (Galera Library 25.3.29):
