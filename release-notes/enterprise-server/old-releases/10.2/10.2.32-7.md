...
* [MariaDB Enterprise Backup](../../10-2/broken-reference/) ignored the timeout setting --ftwrl-wait-timeout when an explicit `LOCK TABLES ... WRITE` was active in another session. As a result, MariaDB Enterprise Server waited for release of the lock even if the timeout was reached. ([MDEV-20230](https://jira.mariadb.org/browse/MDEV-20230))
