...
This eighth release of [MariaDB Enterprise Server]({release-notes}/enterprise-server) 10.5 is a maintenance release. This release includes a variety of fixes.
...
* MariaDB Enterprise Backup expects [--stream=mbstream]({server}/server-usage/backup-and-restore/mariadb-backup/mariadb-backup-options#stream) to stream a backup using the included `mbstream` utility. ([MDEV-15730](https://jira.mariadb.org/browse/MDEV-15730))
  * Prior to this release, MariaDB Enterprise Backup expected [--stream=xbstream]({server}/server-usage/backup-and-restore/mariadb-backup/mariadb-backup-options#stream), even though the `xbstream` utility had been renamed to `mbstream`.
...
* MariaDB Enterprise Backup crashes when executed with the [--backup]({server}/server-usage/backup-and-restore/mariadb-backup/mariadb-backup-options#backup) and [--databases-exclude]({server}/server-usage/backup-and-restore/mariadb-backup/mariadb-backup-options#databases-exclude) options. ([MDEV-25815](https://jira.mariadb.org/browse/MDEV-25815))
...
* When an unknown column is referenced in the `WHERE` clause of a recursive CTE, the query sometimes succeeds instead of failing with an [ER\_BAD\_FIELD\_ERROR]({server}/reference/error-codes/mariadb-error-codes-1000-to-1099/e1054). ([MDEV-26189](https://jira.mariadb.org/browse/MDEV-26189))
* Reusing CTE inside a function fails with the [ER\_NO\_SUCH\_TABLE]({server}/reference/error-codes/mariadb-error-codes-1100-to-1199/e1146) error code. ([MDEV-23886](https://jira.mariadb.org/browse/MDEV-23886))
...
* For [INSERT .. RETURNING]({server}/reference/sql-statements/data-manipulation/inserting-loading-data/insertreturning) statements, a qualified asterisk (such as `table_name.*`) is not supported in the RETURNING clause. ([MDEV-23178](https://jira.mariadb.org/browse/MDEV-23178))
