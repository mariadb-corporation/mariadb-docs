...
MariaDB Enterprise Server 10.5.17-12 is a maintenance release of [MariaDB Enterprise Server]({release-notes}/enterprise-server) 10.5. This release includes a variety of fixes.
...
* When [mariadb-backup]({server}/server-usage/backup-and-restore/mariadb-backup) is executed with the [--compress]({server}/server-usage/backup-and-restore/mariadb-backup/mariadb-backup-options#compress) and [--parallel]({server}/server-usage/backup-and-restore/mariadb-backup/mariadb-backup-options#parallel) options, the backup can hang due to a race condition between threads.
...
* When [FLUSH BINARY LOGS]({server}/reference/sql-statements/administrative-sql-statements/flush-commands/flush) is executed, the server can hang.
...
* When a backup is performed with [mariadb-backup]({server}/server-usage/backup-and-restore/mariadb-backup), the backup includes binary logs.
...
* When [mariadb client]({server}/clients-and-utilities/mariadb-client/mariadb-command-line-client) uses `EditLine` instead of readline (such as on Debian and Ubuntu), Unicode characters are not accepted.
...
* [mariadb-backup]({server}/server-usage/backup-and-restore/mariadb-backup) --sst-max-binlogs command-line option removed
