...
MariaDB Enterprise Server 10.5.16-11 is a maintenance release of [MariaDB Enterprise Server]({release-notes}/enterprise-server) 10.5. This release includes a variety of fixes.
...
* When [ANALYZE FORMAT=JSON]({server}/reference/sql-statements/administrative-sql-statements/analyze-and-explain-statements/analyze-format-json) is executed with a query that contains a subquery, the server can crash. ([MDEV-28268](https://jira.mariadb.org/browse/MDEV-28268))
...
* When [OPTIMIZE TABLE](https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/ha-and-performance/optimization-and-tuning/optimizing-tables/optimize-table) is executed on a sequence, the server raises an [ER\_BINLOG\_UNSAFE\_STATEMENT]({server}/reference/error-codes/mariadb-error-codes-1500-to-1599/e1592) warning, even if `binlog_format` is set to `ROW` or `MIXED`. ([MDEV-24617](https://jira.mariadb.org/browse/MDEV-24617))
...
* The server and [MariaDB Enterprise Backup]({server}/server-usage/backup-and-restore/mariadb-backup) can raise unnecessary warnings about tablespace IDs in some scenarios. ([MDEV-27343](https://jira.mariadb.org/browse/MDEV-27343))
...
* This warning can be written to standard output (`stdout`) when preparing a backup with [MariaDB Enterprise Backup]({server}/server-usage/backup-and-restore/mariadb-backup).
...
* Starting with this release, the server will raise the [WARN\_OPTION\_IGNORED warning]({server}/reference/error-codes/mariadb-error-codes-1600-to-1699/e1618).
...
* When a Spider table is queried using `IF(COUNT() ..)`, the server can raise an [ER\_BAD\_FIELD\_ERROR error]({server}/reference/error-codes/mariadb-error-codes-1000-to-1099/e1054). ([MDEV-25116](https://jira.mariadb.org/browse/MDEV-25116))
