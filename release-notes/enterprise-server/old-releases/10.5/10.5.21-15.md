...
* Starting with this release, the messages about log scanning are only printed when [--verbose]({server}/server-usage/backup-and-restore/mariadb-backup/mariadb-backup-options#-verbose) is enabled.
...
* When [innodb\_undo\_directory](https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/server-usage/storage-engines/innodb/innodb-system-variables#innodb_undo_directory) is set to a relative path, the path is not properly used by [mariadb-backup --copy-back]({server}/server-usage/backup-and-restore/mariadb-backup/mariadb-backup-options#-copy-back). ([MDEV-28187](https://jira.mariadb.org/browse/MDEV-28187))
...
* When an `UPDATE` contains a `WHERE` clause that contains a range condition over a non-indexed VARCHAR column, an error is raised. ([MDEV-20773](https://jira.mariadb.org/browse/MDEV-20773))
  * In previous releases, an error with the [ER\_DATA\_TOO\_LONG error code]({server}/reference/error-codes/mariadb-error-codes-1400-to-1499/e1406) is raised with the following error message:
