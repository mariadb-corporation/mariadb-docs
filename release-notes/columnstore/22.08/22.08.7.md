...
[MariaDB ColumnStore]({analytics}/mariadb-columnstore) 22.08.7 is a maintenance release of [MariaDB ColumnStore]({analytics}/mariadb-columnstore). MariaDB ColumnStore is a columnar storage engine included with MariaDB Enterprise Server.
...
* When a CTE contains a query that joins an [InnoDB](https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/server-usage/storage-engines/innodb) table and a [ColumnStore](../) table, if the CTE results contain a [TIMESTAMP column]({server}/reference/data-types/date-and-time-data-types/timestamp), the column can sometimes be returned as a zero-date (which is 0000-00-00 00:00:00). ([MCOL-5311](https://jira.mariadb.org/browse/MCOL-5311))
