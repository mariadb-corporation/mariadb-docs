...
thread count can be customized in monitor settings. See [MariaDB Monitor documentation]({maxscale}/reference/maxscale-monitors/mariadb-monitor)
...
[Binlogrouter]({maxscale}/reference/maxscale-routers/maxscale-binlogrouter)
...
hostnames/IPs from which admin (REST-API) clients can log in from. See [admin\_readonly\_hosts]({maxscale}/maxscale-management/deployment/installation-and-configuration/maxscale-configuration-guide) and [admin\_readwrite\_hosts]({maxscale}/maxscale-management/deployment/installation-and-configuration/maxscale-configuration-guide)
for more information.
...
used by MariaDB Monitor to detect and set up replication. See [MariaDB Monitor documentation]({maxscale}/reference/maxscale-monitors/mariadb-monitor)
for more information.

#### [MXS-4764](https://jira.mariadb.org/browse/MXS-4764) KafkaCDC: Option to use the latest GTID

The [gtid]({maxscale}/reference/maxscale-routers/maxscale-kafkacdc) parameter now supports the special
...
For more information, please refer to the [Limitations]({maxscale}/maxscale-management/mariadb-maxscale-limitations-guide) document.
