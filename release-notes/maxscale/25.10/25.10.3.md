...
## Upgrading From Older Versions When Using Configuration Synchronization

If configuration synchronization is used and you are upgrading from an older
major version of MaxScale, due to
[MXS-6366](https://jira.mariadb.org/browse/MXS-6366), you must first upgrade to
the latest patch version of the old release, do a configuration change that is
synchronized into the cluster and only then upgrade the major version to 25.10.
...
Although a maintenance release, MaxScale 25.10.3 introduces some significant
new functionality that, in order to not change the dependencies of exiting
packages, are delivered in packages of their own.

### maxscale-exasol

Contains [exasolrouter](https://app.gitbook.com/s/0pSbu5DcMSW4KwAkUcmX/reference/maxscale-routers/maxscale-exasolrouter), a MaxScale router that is capable of routing requests to an Exasol Analytics Engine.
