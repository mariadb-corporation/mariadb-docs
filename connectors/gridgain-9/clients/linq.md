---
description: >-
  Write strongly typed queries against GridGain 9 with the .NET client's LINQ
  provider, which translates C# LINQ expressions into GridGain SQL.
---
...
In real-world scenarios the performance of GridGain LINQ queries is on par with equivalent SQL queries.
...
The following `Math` functions are NOT supported (no equivalent in GridGain SQL engine):
