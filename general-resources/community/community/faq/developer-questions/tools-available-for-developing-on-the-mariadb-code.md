...
[Building the code from source]({server}/server-management/install-and-upgrade-mariadb/compiling-mariadb-from-source) is done with standard Unix tools: CMake (or autotools for MariaDB versions below 5.5), Gnu Make, GCC (or other C/C++ compiler on some systems). On [Windows]({server}/server-management/install-and-upgrade-mariadb/compiling-mariadb-from-source/building_mariadb_on_windows), CMake and Visual Studio are used.
...
The [source]({server}/server-management/install-and-upgrade-mariadb/compiling-mariadb-from-source) page has links to instructions on setting up a full development environment, if you are interested.
