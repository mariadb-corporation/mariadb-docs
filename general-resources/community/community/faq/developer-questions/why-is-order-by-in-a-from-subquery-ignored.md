...
A "table" (and subquery in the `FROM` clause too) is - according to the SQL standard - an unordered set of rows. Rows in a table (or in a subquery in the `FROM` clause) do not come in any specific order. That's why the optimizer can ignore the [ORDER BY]({server}/reference/sql-statements/data-manipulation/selecting-data/order-by) clause that you have specified. In fact, the SQL standard does not even allow the `ORDER BY` clause to appear in this subquery (we allow it, because `ORDER BY ... LIMIT ...` changes the result, the set of rows, not only their order).
...
* [SELECT]({server}/reference/sql-statements/data-manipulation/selecting-data/select)
* [ORDER BY]({server}/reference/sql-statements/data-manipulation/selecting-data/order-by)
