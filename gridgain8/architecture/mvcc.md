---
description: >-
  Multiversion concurrency control (MVCC) in GridGain 8, enabled through the
  TRANSACTIONAL_SNAPSHOT atomicity mode, its limitations, and how to use it.
---

# Multiversion Concurrency Control

{% hint style="warning" %}
MVCC support is in beta and is not recommended for production use.
{% endhint %}
...
Multiversion Concurrency Control (MVCC) is a method of controlling the consistency of data accessed by multiple users concurrently. MVCC implements the [snapshot isolation](https://en.wikipedia.org/wiki/Snapshot_isolation) guarantee which ensures that each transaction always sees a consistent snapshot of data.

Each transaction obtains a consistent snapshot of data when it starts and can only view and modify data in this snapshot.
When the transaction updates an entry, GridGain verifies that the entry has not been updated by other transactions and creates a new version of the entry.
The new version becomes visible to other transactions only when and if this transaction commits successfully.
If the entry has been updated, the current transaction fails with an exception (see the [Concurrent Updates](#concurrent-updates) section for the information on how to handle update conflicts).
