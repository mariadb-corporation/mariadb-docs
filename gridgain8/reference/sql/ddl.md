---
description: >-
  Reference of the SQL Data Definition Language (DDL) commands supported by
  GridGain — CREATE/ALTER/DROP TABLE, indexes, users, and statistics commands.
---
...
You can also use the CREATE TABLE statement for exposing existing cache to SQL engine. To do this you need to define all desired columns (column names should match field name of underlying cache key/value class) and specify CACHE_NAME, KEY_TYPE, VALUE_TYPE parameters. For example:
...
WITH "
    CACHE_NAME=ORGANIZATION,
    KEY_TYPE=org.acme.PersonKey,
    VALUE_TYPE=org.acme.Person
";
```
...
- `LOGGING` - enable [write-ahead logging](../../architecture/storage/native-persistence.md#write-ahead-log-wal) for the table. Write-ahead logging in enabled by default. The command is relevant only if Ignite persistence is used.
- `NOLOGGING` - disable write-ahead logging for the table. The command is relevant only if Ignite persistence is used.

`ALTER TABLE ADD` adds a new column or several columns to a previously created table. Once a column is added, it can be accessed using [DML commands](dml.md) and indexed with the [CREATE INDEX](#create-index) statement.
...
Schema changes applied by this command are persisted on disk if [Ignite persistence](../../architecture/storage/native-persistence.md) is enabled. Thus, the changes can survive full cluster restarts.
...
{% hint style="info" %}
Created index always includes the primary key of the table, even if it is not stated explicitly.
{% endhint %}
...
- You can use either `TEST` or `"Test"` as the username from GridGain's native SQL APIs designed for Java, .NET and other programming languages.
...
The password can be updated by the superuser (`ignite`, see [CREATE USER](#create-user) for more details) or by the user themselves.
...
The user can be removed only by the superuser (`ignite`, see [CREATE USER](#create-user) for more details).
...
```sql
ANALYZE public.statistics_test, statistics_test2, statistics_test3(col3)
```
