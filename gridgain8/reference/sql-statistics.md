---
description: >-
  How GridGain collects and uses SQL statistics to build optimal query plans,
  including configuration, overriding, and obsolescence tracking.
---
...
Without statistics, SQL query execution planner tries to guess about the query conditions selectivity using only general heuristics. To get better plans, make sure that statistics usage is [enabled](cli-tool/README.md) and [configure statistics collection](sql/ddl.md#analyze) for tables participated in the query. See the [example](#obtaining-a-better-execution-plan-using-statistics) for more details.
...
For more details on configuring SQL statistics, see the [System Views](monitoring/system-views.md) and [Data Definition Language](sql/ddl.md) pages.

## Statistics Overriding

Gathered values can be overridden by specifying additional parameters in the [ANALYZE](sql/ddl.md#analyze) command. The specified values override gathered one on each node in [STATISTICS_LOCAL_DATA](monitoring/system-views.md#statistics_local_data), but not in [STATISTICS_PARTITION_DATA](monitoring/system-views.md#statistics_partition_data). Then, overridden values are used by SQL query optimizer.
