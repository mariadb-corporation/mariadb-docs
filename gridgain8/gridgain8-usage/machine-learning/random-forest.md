...
Random forest is an ensemble learning method to solve any classification and regression problem. Random forest training builds a model composition (ensemble) of one type and uses some aggregation algorithm of several answers from models. Each model is trained on a part of the training dataset. The part is defined according to bagging and feature subspace methods. More information about these concepts may be found here: [1](https://en.wikipedia.org/wiki/Random_forest), [2](https://en.wikipedia.org/wiki/Bootstrap_aggregating), and [3](https://en.wikipedia.org/wiki/Random_subspace_method).
