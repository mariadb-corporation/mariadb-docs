...
#### Scenario 1: Local Development (Linux/Windows)
...
# Services
DOCLING_RAY_SERVICE_URL=http://localhost:8003
OLLAMA_HOST=http://localhost:11434

# Storage
FILE_STORAGE_TYPE=local
LOCAL_STORAGE_PATH=./uploaded_files
...
#### Scenario 2: Docker Deployment (Linux)

```ini
# Database (cloud or docker service)
DB_HOST=mariadb
DB_PORT=3306

# Servers (docker service names)
APP_HOST=rag-api
MCP_HOST=mcp-server
MCP_MARIADB_HOST=mariadb

# Services (docker service names)
DOCLING_RAY_SERVICE_URL=http://docling-ray:8003
OLLAMA_HOST=http://ollama:11434

# Storage
FILE_STORAGE_TYPE=s3
MANAGED_S3_BUCKET=rag-staging-bucket
MANAGED_S3_ENDPOINT_URL=http://minio:9000
...
#### Scenario 3: Docker Deployment on macOS

macOS Docker Desktop uses `host.docker.internal` to access services on the host machine. For inter-container communication, use service names.
...
# Services - USE host.docker.internal FOR HOST SERVICES
DOCLING_RAY_SERVICE_URL=http://docling-ray:8003
...
FILE_STORAGE_TYPE=s3
MANAGED_S3_BUCKET=rag-staging-bucket
MANAGED_S3_ENDPOINT_URL=http://host.docker.internal:9000

# Redis (docker service) - USE SERVICE NAME, NOT host.docker.internal
CELERY_BROKER_URL=redis://redis:6379/0
CELERY_RESULT_BACKEND=redis://redis:6379/0
REDIS_URL=redis://redis:6379/0
```
...
* Use `host.docker.internal` ONLY for services running on the macOS host (Ollama, MinIO on host).
* Use service names (e.g., `redis`, `mariadb`) for services running in Docker containers.
* If you get "All connection attempts failed" error, check that:
  1. The service is actually running on the host (e.g., `ollama serve` is running).
  2. You're using `host.docker.internal` for host services, not `localhost`.
  3. You're using service names for Docker services, not `host.docker.internal`.
