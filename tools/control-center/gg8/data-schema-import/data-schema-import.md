...
The **Data and Schema Import** feature lets you migrate database schemas and data from external JDBC-compatible databases (PostgreSQL, MySQL, and others) or Apache Iceberg into a GridGain 8 cluster. It generates `DDL` statements for table creation and `COPY` statements for data migration.
...
- The **Queries** [screen](../queries/querying.md) — via the **Import Schema**/**Import Data** action button.
...
{% hint style="success" %}
You can open **Data Import** directly from the **Queries** screen without going through the schema setup steps. Do this when the target tables already exist in the GridGain cluster and you only need to load or reload data.
{% endhint %}
...
![Connection step — JDBC](../../../.gitbook/assets/cc-gg8-import-step1-jdbc.png)
...
![Connection step — Apache Iceberg REST](../../../.gitbook/assets/cc-gg8-import-step1-iceberg.png)
...
![Schemas & Tables step](../../../.gitbook/assets/cc-gg8-import-step2-tables.png)

The imported schema will be loaded into one of the existing GridGain 8 clusters.
...
Select the GridGain 8 cluster into which the schema will be imported. Only clusters in an active state are available. If no active cluster is available, click **Save and Close** to save your configuration and resume the import when a cluster becomes available.
...
- **Merge with existing schema** is set by default. When selected, the imported tables are added to the existing schema. Existing tables and data are not affected.
  ...
  ![Conflict Resolution](../../../.gitbook/assets/cc-gg8-import-step3-conflict-resolve-merge.png)
- When **Rename schema** is selected, the imported schema is renamed by adding a configurable prefix and/or suffix, avoiding any conflict with the existing schema.
  ...
  ![Table Settings step — rename conflict resolution](../../../.gitbook/assets/cc-gg8-import-step3-conflict-resolve-rename.png)
- When **Drop and recreate** is selected, note that **all existing data will be lost**:

  ![Table Settings step — drop and recreate conflict resolution](../../../.gitbook/assets/cc-gg8-import-step3-conflict-resolve-drop.png)
...
|---|---|
| Primary key | The column(s) that uniquely identify each record and determine how data is distributed across the cluster. |
| Cache group | The name of the cache group the underlying cache belongs to. Caches in the same group share internal storage structures to reduce overhead. |
| Template | The cache template used to create the underlying cache — `PARTITIONED` or `REPLICATED` — controlling how data is distributed. |
| Atomicity | The atomicity mode of the underlying cache: `ATOMIC` or `TRANSACTIONAL`. Determines the write mode for the table. If not set, `ATOMIC` is used. |
| Data region | The data region in which the table's data is stored. |
| Affinity key | A column from the primary key used to colocate related rows on the same nodes for faster joins. |
| Affinity inline size | The inline size, in bytes, for the affinity field index. Leave empty to use the default. |
| PK inline size | The inline size, in bytes, for the primary key index. Leave empty to use the default. |
| Key type | The name of the key type for the underlying cache. If not set, a type name is generated automatically. |
| Value type | The name of the value type for the underlying cache. If not set, a type name is generated automatically. |
| Write synchronization mode | The write synchronization mode for the underlying cache: `FULL_SYNC`, `FULL_ASYNC`, or `PRIMARY_SYNC`. |
...
{% hint style="success" %}
You can leave these fields empty to use cluster defaults. This is useful when you want to import the schema quickly and tune storage settings later.
{% endhint %}
...
![Column Settings step](../../../.gitbook/assets/cc-gg8-import-step4-column-settings.png)

Each table can be expanded to inspect its columns. For each column, the wizard shows the source name and type and lets you review and adjust the GridGain 8 mapping:
...
![Column Settings step — column type error](../../../.gitbook/assets/cc-gg8-import-step4-column-settings-error.png)

Each table can also be expanded to review its indexes. For each index you can configure:

- **Name** — the name of the index.
- **Columns** — the columns included in the index.
- **Inline size** — the optional index inline size, in bytes.
- **Parallel** — the optional level of parallelism used to build the index.

For each column in the index, set:

- **Column** — the indexed column.
- **Direction** — the sort order, `ASC` or `DESC`.

Reorder the index columns by dragging them using the handle on the left.

![Column Settings step — index](../../../.gitbook/assets/cc-gg8-import-step4-column-settings-index.png)

## Data Import

This step is shown when **Import data after schema is created** is enabled on the [Connection](#connection) step. When the toggle is on, `COPY` statements are generated and included in the script alongside the `DDL` for execution in the final step.
...
![Data Import step — source/target](../../../.gitbook/assets/cc-gg8-import-step5-source.png)
...
![Data Import step — additional params](../../../.gitbook/assets/cc-gg8-import-step5-additional-params.png)

## Script Overview

This tab displays the generated scripts to review and edit before execution.

![Overview](../../../.gitbook/assets/cc-gg8-import-step6-overview.png)
...
- Open the script in **SQL Notebook**: Control Center redirects you to the **Queries** [screen](../queries/querying.md), where this script will be available for editing. After you execute the script, the corresponding tables will be created or updated.

You can track execution progress and results in the **Queries log** tab. If an error occurs, it will also appear there with detailed logs.
...
![Import Schemas menu option](../../../.gitbook/assets/cc-gg8-import-schemas-menu.png)
...
![Import Schemas screen](../../../.gitbook/assets/cc-gg8-import-schemas-screen.png)
...
- **Open in SQL Notebook** — open the generated script in the [Queries](../queries/querying.md) screen for review or manual execution.
