...
- **Graceful Shutdown Switchover** is enabled by default in `26.6.0` If you do not want primary switchovers to happen automatically when a primary Pod is gracefully terminated, you must disable the feature on your `MariaDB` resources before updating the operator. Refer to the [Graceful Shutdown Switchover migration guide](../migrations/migrate-graceful-shutdown-switchover-26-06.md) for instructions.
