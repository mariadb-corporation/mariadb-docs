...
| `ROLLBACK TO SAVEPOINT sp1;` followed by re-using a savepoint set *after* `sp1` | Rolling back to a savepoint **erases every savepoint created after it**. Locks acquired after the savepoint are *not* released by the rollback |
