...
MaxScale uses monitors to retrieve additional information from the servers. This information is used by other services in filtering and routing connections based on the current state of the node. For Enterprise Cluster, use the [Galera Monitor (galeramon)]({maxscale}/reference/maxscale-monitors/galera-monitor).
