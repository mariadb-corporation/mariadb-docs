...
## Enabling TLS on a Replica

TLS can be enabled on a replication replica by issuing the [CHANGE MASTER](../../../reference/sql-statements/administrative-sql-statements/replication-statements/change-master-to.md) statement, while setting a number of options. Which options you set depends on whether you want one-way TLS or two-way TLS, and whether or not you want to verify the server certificate.
...
#### Enabling One-Way TLS With Server Certificate Verification
...
#### Enabling One-Way TLS Without Server Certificate Verification
...
If the [replica threads](../../../ha-and-performance/standard-replication/replication-threads.md#threads-on-the-replica) are currently running, you first need to stop them by issuing the [STOP REPLICA](../../../reference/sql-statements/administrative-sql-statements/replication-statements/stop-replica.md) statement:
...
Now start replication by issuing the [START REPLICA](../../../reference/sql-statements/administrative-sql-statements/replication-statements/start-replica.md) statement:

```sql
START REPLICA;
```

The replica now uses TLS to encrypt data in transit as it replicates from the primary.
...
Once the server is back online, set the [MASTER\_SSL](../../../reference/sql-statements/administrative-sql-statements/replication-statements/change-master-to.md#master_ssl) option by executing the [CHANGE MASTER](../../../reference/sql-statements/administrative-sql-statements/replication-statements/change-master-to.md) statement. This enables TLS. For example:

```sql
CHANGE MASTER TO MASTER_SSL=1;
```

The certificate and keys are read from the option file. At this point, you can start replication by issuing the [START REPLICA](../../../reference/sql-statements/administrative-sql-statements/replication-statements/start-replica.md) statement:
