---
description: >-
  Overview of the feature introduced in MariaDB 11.5 to limit disk space used by
  temporary files and internal on-disk temporary tables to prevent disk
  exhaustion.
---
...
{% hint style="info" %}
This functionality is available from MariaDB 11.5.
{% endhint %}

You can limit the size of created disk temporary files and tables.\
...
* [max\_tmp\_session\_space\_usage](max_tmp_session_space_usage-system-variable.md): Limits the temporary space that a single connection can use.
* [max\_tmp\_total\_space\_usage](max_tmp_total_space_usage-system-variable.md): Limits the temporary space that all connections can use together.

{% hint style="info" %}
Both limits are checked on every write that grows a temporary file or temporary table. The global limit is checked first: exceeding it fails the statement with `Global temporary space limit reached`, while exceeding the connection's own limit fails it with `Local temporary space limit reached`. Setting either variable to 0 disables that limit only.
{% endhint %}
...
* One has to be careful when using small values for either temporary space limit together with [binary logging](../../server-management/server-monitoring-logs/binary-log/) and with non transactional tables.
* If a binary log entry for the query is larger than [binlog\_stmt\_cache\_size](../../ha-and-performance/standard-replication/replication-and-binary-log-system-variables.md#binlog_stmt_cache_size) and one hits either temporary space limit when flushing the entry to disk, the query will abort and the binary log will not contain the last changes to the table. This will also stop the replica!
...
* global\_tmp\_space\_used contains the total tmp space used so far. This is needed to quickly check against max\_tmp\_total\_space\_usage.
...
* As a precaution against errors in the tmp\_space\_used code, one can set max\_tmp\_session\_space\_usage and max\_tmp\_total\_space\_usage to 0 to disable the tmp space quota errors.
