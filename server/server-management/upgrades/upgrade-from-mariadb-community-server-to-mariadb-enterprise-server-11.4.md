...
1.  Use the `information\_schema.INNODB\_SYS\_TABLES` to identify any InnoDB tables that use the `COMPRESSED` row format:
    ...
    ```
2.  Execute an [ALTER TABLE](../../reference/sql-statements/data-definition/alter/alter-table/README.md) statement for each table, changing its row format from `COMPRESSED` to `DYNAMIC`:
