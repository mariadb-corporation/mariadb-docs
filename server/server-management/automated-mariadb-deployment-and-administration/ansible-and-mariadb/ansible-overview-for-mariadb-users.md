...
* A `mariadb-backup` role to take backups with [mariadb-backup](../../../server-usage/backup-and-restore/mariadb-backup/), running jobs during the night. We can associate this role to the "`db-main`" group, or we could create a child group for servers that will take the backups.
* `mariadb-dump` for the server that takes dumps with [mariadb-dump](../../../clients-and-utilities/backup-restore-and-import-clients/mariadb-dump.md). Note that we may decide to take dumps on a replica, so the same host may belong to `db-analytics` and `mariadb-dump`.
