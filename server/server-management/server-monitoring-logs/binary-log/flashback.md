...
Flashback is supported only for DML statements ([INSERT](../../../reference/sql-statements/data-manipulation/inserting-loading-data/insert.md), [DELETE](../../../reference/sql-statements/data-manipulation/changing-deleting-data/delete.md), [UPDATE](../../../reference/sql-statements/data-manipulation/changing-deleting-data/update.md)). Flashback for DDL statements ([DROP](../../../reference/sql-statements/data-definition/drop/drop-table.md), [TRUNCATE](../../../reference/sql-statements/table-statements/truncate-table.md), [ALTER](../../../reference/sql-statements/data-definition/alter/alter-table/), etc.) is not supported. See [MDEV-10571](https://jira.mariadb.org/browse/MDEV-10571).
...
The real work of Flashback is done by [mariadb-binlog](../../../clients-and-utilities/logging-tools/mariadb-binlog/) with [`--flashback`](../../starting-and-stopping-mariadb/mariadbd-options.md#flashback). This causes events to be translated: `INSERT` to `DELETE`, `DELETE` to `INSERT`, and for `UPDATE` statements, the before and after images are swapped.
...
With a _`mytable`_ table in a _`test`_ database, you can compare the output with `--flashback` and without:
