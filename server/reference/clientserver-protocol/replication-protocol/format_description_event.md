...
* [byte\<n>](../protocol-data-types.md#fixed-length-bytes) Variable-sized. An array that indicates the post-header lengths for all event types. There is one byte per event type that the server knows about. The value 'n' comes from the following formula:
