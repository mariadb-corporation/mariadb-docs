...
The SQL statement should be properly escaped. The escape character is usually a backslash (`\`) = `0x5c`. However, if the status flag returned by the last [OK Packet](../4-server-response-packets/ok_packet.md#server-status-flag) had the `NO_BACKSLASH_ESCAPES` bit set, the escape character is a single quote (`'` = `0x60`).
