...
| USER              | MariaDB User.                                                                                                                                                                                                                                                                                                                                            |
| HOST              | The hostname from which this client is connected. For Unix socket connections, this is `localhost`. For TCP/IP connections, the TCP port is appended (for instance, `192.168.1.17:58061` or `other-host.company.com:58061`). For the system user, and whenever the server is started with `--skip-grant-tables`, this column is blank (`''`).                                                                            |
...
{% hint style="warning" %}
When the server is started with `--skip-grant-tables`, the `HOST` column is an empty string (`''`) for every session, over both TCP/IP and Unix sockets, and no TCP port is appended.

[SHOW PROCESSLIST](../../../sql-statements/administrative-sql-statements/show/show-processlist.md) does not behave the same way — its `Host` column falls back to the connection's hostname, so the two disagree. The exception is a TCP connection to a server also started with `--skip-name-resolve`, where the hostname was never resolved and `SHOW PROCESSLIST` shows an empty `Host` too.

Filtering on `HOST` is therefore unreliable while the server runs with `--skip-grant-tables`.
{% endhint %}
