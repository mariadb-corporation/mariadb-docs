...
In [row-based](../../../server-management/server-monitoring-logs/binary-log/binary-log-formats.md#row-based-logging) [replication](../../../ha-and-performance/standard-replication/), only the master checks constraints, and failed statements are not replicated. In [statement-based](../../../server-management/server-monitoring-logs/binary-log/binary-log-formats.md#statement-based-logging) replication, the slaves also check constraints. Constraints should therefore be identical, as well as deterministic, in a replication environment.
