...
* Flexibility. Let's say we need to see all clients even if they don't have a phone number in the system. With the second version, it's easy; just change `INNER JOIN phoneNumbers` to `LEFT JOIN phoneNumbers`. Try that with the first version, and MariaDB will raise an `ER_BAD_FIELD_ERROR` ("Unknown column") error when the query executes, because the change in precedence between the comma operator and the `JOIN` keyword means the `ON` clause can no longer resolve columns from the comma-joined table. The solution is to rearrange the `FROM` clause or add parentheses to override the precedence, and that quickly becomes frustrating.
