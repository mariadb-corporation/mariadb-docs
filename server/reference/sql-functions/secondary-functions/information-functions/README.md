---
description: >-
  Retrieve system and session metadata. This section details functions to access
  database names, user details, version info, and query statistics like row
  counts.
---

# Information Functions

- [BENCHMARK](benchmark.md)
- [BINLOG\_GTID\_POS](binlog_gtid_pos.md)
- [CHARSET](charset.md)
- [COERCIBILITY](coercibility.md)
- [COLLATION](collation.md)
- [CONNECTION\_ID](connection_id.md)
- [CURRENT\_ROLE](current_role.md)
- [CURRENT\_USER](current_user.md)
- [DATABASE](database.md)
- [DECODE\_HISTOGRAM](decode_histogram.md)
- [DEFAULT](default.md)
- [FOUND\_ROWS](found_rows.md)
- [LAST\_INSERT\_ID](last_insert_id.md)
- [LAST\_VALUE](last_value.md)
- [PROCEDURE ANALYSE](procedure-analyse.md)
- [ROW\_COUNT](row_count.md)
- [ROWNUM](rownum.md)
- [SCHEMA](schema.md)
- [SESSION\_USER](session_user.md)
- [SYSTEM\_USER](system_user.md)
- [USER](user.md)
- [VERSION](version.md)

<sub>_This page is licensed: CC BY-SA / Gnu FDL_</sub>
