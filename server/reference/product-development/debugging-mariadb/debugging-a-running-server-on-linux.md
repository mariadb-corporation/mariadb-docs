...
Even if you don't have a server that is [compiled for debugging](compiling-mariadb-for-debugging.md), there are still ways to get more information out of it if things go wrong.

When things go wrong, it's always better to have a version of the mysqld daemon that is not stripped.
...
Now start your client/application that uses MariaDB. You can find where memory is allocated in the `/tmp/mysqld.prof` file. If you find any memory issues, please report them in the [MariaDB bug tracker](https://jira.mariadb.org/secure/Dashboard.jspa)!
