...
If purge operations are lagging on a busy server, then this can be a tough situation to recover from. As a solution, InnoDB allows you to set the max purge lag. The max purge lag is defined as the maximum number of [InnoDB undo logs](innodb-undo-log.md) that can be waiting to be purged from the history until InnoDB begins delaying DML statements.
...
If a row's last [InnoDB undo log](innodb-undo-log.md) record is purged, this can obviously affect the value of the row's `DB_ROLL_PTR` column, because there would no longer be any [InnoDB undo log](innodb-undo-log.md) record for the pointer to reference.
