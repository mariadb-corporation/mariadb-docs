...
#
# The four exclusions below were carried in codespell.yml's files_ignore (DOCS-6206, DOCS-6369,
# DOCS-6618) but never mirrored here, so a local commit touching one of these paths failed a
# check CI itself does not run — the exact "ambush" DOCS-6615 describes, just one hop further
# down the same latent-drift class the SOURCE-OF-TRUTH header above warns about.
spell_files=()
for f in "${files[@]}"; do
  case "$(basename "$f")" in
    SUMMARY.md) continue ;;
  ...
  case "$f" in
    # Vendored verbatim from MariaDB/skills (agent-skills/topical/VENDORED.md); spelling fixes
    # go upstream, not here. DOCS-6206.
    agent-skills/topical/*) continue ;;
    # Changelog pages transcribe upstream MDEV/git commit messages verbatim, so their
    # misspellings belong to the commit history, not to us -- "correcting" them rewrites a
    # quotation. DOCS-6369, DOCS-6615.
    release-notes/*changelogs/*|release-notes/*-changelogs/*| \
    release-notes/*changelog-*.md|release-notes/*-changelog.md) continue ;;
    # Generated by doxygen+moxygen from the server's own headers; "misspellings" are C
    # identifiers and macro parameter names quoted from source. DOCS-6618.
    server/reference/plugins/api-plugin/*) continue ;;
  esac
  spell_files+=("$f")
done
