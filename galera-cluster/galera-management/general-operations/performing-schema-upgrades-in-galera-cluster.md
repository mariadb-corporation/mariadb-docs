...
| Rolling Schema Upgrade (RSU) | Advanced, [non-blocking method({galera}/configuration/galera-cluster-address)]. The DDL is executed on the local node, with changes applied manually to each node in sequence, keeping the cluster online. |
