...
* [log_slave_updates]({server}/ha-and-performance/standard-replication/replication-and-binary-log-system-variables#log_slave_updates) needs to be enabled on all nodes in the cluster. See [MDEV-9855](https://jira.mariadb.org/browse/MDEV-9855) about that.
* [log_bin]({server}/ha-and-performance/standard-replication/replication-and-binary-log-system-variables) needs to be set to the same path on all nodes in the cluster. See [MDEV-9856](https://jira.mariadb.org/browse/MDEV-9856) about that.
...
Our process to set up replication is going to be similar to the process described at [Setting up a Replication Slave with mariadb-backup]({server}/server-usage/backup-and-restore/mariadb-backup/setting-up-a-replica-with-mariadb-backup), but it will be modified a bit to work in this context.
...
The first step is to simply take and prepare a fresh [full backup]({server}/server-usage/backup-and-restore/mariadb-backup/full-backup-and-restore-with-mariadb-backup) of the node that you have chosen to be the replication primary. For example:
...
At this point, you can restore the backup to the [datadir]({server}/server-management/variables-and-modes/server-system-variables#datadir), as you normally would. For example:
...
Before the MariaDB Server replica can begin replicating from the cluster's primary, you need to [create a user account]({server}/reference/sql-statements/account-management-sql-statements/create-user) on the primary that the replica can use to connect, and you need to [grant]({server}/reference/sql-statements/account-management-sql-statements/grant) the user account the [REPLICATION SLAVE]({server}/reference/sql-statements/account-management-sql-statements/grant#global-privileges) privilege. For example:
...
The coordinates will be in the [xtrabackup\_binlog\_info]({server}/server-usage/backup-and-restore/mariadb-backup/files-created-by-mariadb-backup#xtrabackup_binlog_info) file.
...
You should be done setting up the replica now, so you should check its status with [SHOW SLAVE STATUS]({server}/reference/sql-statements/administrative-sql-statements/show/show-replica-status). For example:
...
Before circular replication can begin, you also need to [create a user account]({server}/reference/sql-statements/account-management-sql-statements/create-user) on the MariaDB Server, since it will be acting as replication primary to the cluster's replica, and you need to [grant]({server}/reference/sql-statements/account-management-sql-statements/grant) the user account the [REPLICATION SLAVE]({server}/reference/sql-statements/account-management-sql-statements/grant#global-privileges) privilege. For example:
...
To get the GTID coordinates on the MariaDB Server you can check [gtid_current_pos]({server}/ha-and-performance/standard-replication/gtid#gtid_current_pos) by executing:

```
SHOW GLOBAL VARIABLES LIKE 'gtid_current_pos';
```

Then on the node acting as replica in the cluster, you can set up replication by setting [gtid\_slave\_pos](https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/ha-and-performance/standard-replication/gtid#gtid_slave_pos) to the GTID that was returned and then executing [CHANGE MASTER TO]({server}/reference/sql-statements/administrative-sql-statements/replication-statements/change-master-to):
...
To get the [binary log](https://app.gitbook.com/s/SsmexDFPv2xG2OTyO5yV/server-management/server-monitoring-logs/binary-log) file and position coordinates on the MariaDB Server, you can execute [SHOW MASTER STATUS]({server}/reference/sql-statements/administrative-sql-statements/show/show-binlog-status):

```
SHOW MASTER STATUS
```

Then on the node acting as replica in the cluster, you would set `master_log_file` and `master_log_pos` in the [CHANGE MASTER TO]({server}/reference/sql-statements/administrative-sql-statements/replication-statements/change-master-to) command. For example:
...
You should be done setting up the circular replication on the node in the first cluster now, so you should check its status with [SHOW SLAVE STATUS]({server}/reference/sql-statements/administrative-sql-statements/show/show-replica-status). For example:
