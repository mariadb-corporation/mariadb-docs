...
If you are using [embedded nodes](../../gridgain9-get-started/embedded-mode.md), any code that is included in the project classpath will also be available to your compute jobs.
...
Examples below assumes that the `NodeNameJob` class has been deployed to the node by using [code deployment](../code-deployment.md).
...
You can tune the .NET sidecar process through the `ignite.compute.dotnet` section of the [node configuration](../../reference/configuration/node-configuration-parameters.md#compute-configuration). The settings are applied as environment variables when the sidecar process is started:
...
6. Use the GridGain CLI command `cluster unit deploy command` to [deploy](../code-deployment.md) the directory to the cluster as a deployment unit. The deployed code will be available on the cluster.
...
- You can call [Java computing jobs](about-distributed-computing.md) from your .NET code, for example:
...
Another way to cancel jobs is by using the SQL [KILL COMPUTE](../../reference/sql/operational-commands.md#kill-compute) command. The job id can be retrieved via the `COMPUTE_JOBS` [system view](../../reference/monitoring/system-views.md).
...
All MapReduce jobs must be submitted to a node that has an appropriate class [deployed](../code-deployment.md). Below is an example of a map reduce job:
