...
When inserting values to the table, you can set expiration time for these values if TTL is enabled in the table. You can set the TTL by using the [CREATE TABLE](../reference/sql/ddl.md#create-table) or [ALTER TABLE](../reference/sql/ddl.md#alter-table-if-exists-table-set-expire-at) statements.
...
For more details on data partitioning, see the following [article](../architecture/storage/data-partitioning.md).
