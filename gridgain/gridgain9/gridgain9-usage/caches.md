...
Caches are created by using the DDL [CREATE CACHE](../reference/sql/ddl.md#create-cache) command, for example:

{% hint style="warning" %}
When creating a cache, it must use the storage that has the `aimem` [storage type](../architecture/storage/engines/in-memory-storage.md). In this example, we assume that the `in-memory` storage profile has already been configured to use the `aimem` storage engine.
{% endhint %}

### Using SQL

You can create a cache from SQL by using the [CREATE CACHE](../reference/sql/ddl.md#create-cache) command:
