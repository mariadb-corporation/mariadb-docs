...
While [Role-Based Authorization](role-based-authorization.md) controls **whether** a user can access a table at all, row-level security controls **which rows within that table** are visible to and changeable by a user.
...
- When a row-level security enabled table is replicated with [**Data Center Replication (DCR)**](../gridgain9-management/data-center-replication/README.md), filtering occurs according to the
...
[`GG-SECURITY-4`](../reference/error-codes/README.md#security-exceptions) error (`Row-level security constraint violation`).
...
(see [User Permissions and Roles](user-permissions-and-roles.md)).
...
[role-based](role-based-authorization.md) privileges; the policies are retained
and take effect again when row-level security is re-enabled.

### Inspecting Policies

All configured policies are listed in the [`SYSTEM.POLICIES` system view](../reference/monitoring/system-views.md#policies).
