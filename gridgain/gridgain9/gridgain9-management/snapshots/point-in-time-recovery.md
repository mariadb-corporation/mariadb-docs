...
By using the point in time recovery (PITR) feature, you can restore cluster status to any point above the [low watermark](../../architecture/storage/low-watermark-and-gc.md) point.
...
- Point in time recovery can only be performed for data above the [low watermark](../../architecture/storage/low-watermark-and-gc.md) (600000ms by default). Older data can only be recovered by restoring a [snapshot](data-snapshots.md).
