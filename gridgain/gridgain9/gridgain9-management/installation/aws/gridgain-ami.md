...
- *Disk space:* If you are launching a cluster with a [persistent storage](../../../architecture/storage/engines/native-persistent-storage.md), provide enough size to accommodate all your data when configuring the instance's volume.
...
GridGain is [installed as RPM](../installing-deb-rpm.md). It provides [systemd service](../installing-deb-rpm.md#running-gridgain-as-a-service) `gridgain9db` which can be managed using systemctl. Because the service is enabled by default, it should start automatically and be up and running soon after the instance launch.

## Configuring Discovery

Cluster nodes launched in different EC2 instances must be able to connect to each other. Please refer to [Network Configuration](../../../reference/configuration/node-configuration-parameters.md#network-configuration) to let cluster nodes discover each other.
