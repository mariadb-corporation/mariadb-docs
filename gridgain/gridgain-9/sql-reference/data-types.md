---
description: >-
  SQL data types available in GridGain 9 — boolean, numeric, character, binary,
  date/time, UUID, and NULL — and how each maps to Java, .NET, and C++ types.
---
...
Minimum value: `00:00:00.000000000`

Maximum value: `23:59:59.999999999`
...
Maximum value: `9999-12-31 05:59:59.999999999`

Minimum value: `0001-01-01 18:00:00`

Mapped to: `LocalDateTime`
...
{% hint style="warning" %}
In SQL, the timestamp with local time zone data type only supports precision up to milliseconds (3 symbols). Any values past the 3rd symbol will be ignored.
{% endhint %}
...
Possible values: The timestamp data type that represents an absolute point in time. The time zone offset is not stored as part of column data. When the value is written or retrieved, it is automatically converted to or from the time zone of the session.
...
| ColumnType | SQL | Java | .NET | C++ |
|---|---|---|---|---|
| TIMESTAMP | TIMESTAMP WITH LOCAL TIME ZONE | Instant | Instant | ignite_timestamp |
...
Use a typed UUID literal:

```sql
uuid 'cf4f1822-2e99-4b93-ae82-7f4ab907aad4'
```

Typed literals are preferred because they avoid extra expression simplification
and enable literal-specific optimizations.

**UUID generation**

Use `rand_uuid()` to generate a random UUID value:

```sql
select rand_uuid(); -- returns a UUID
```

**Explicit conversion to UUID**

String literals are `CHAR(...)` values and are **not** implicitly converted to `UUID`.
Convert them explicitly:

```sql
CAST('cf4f1822-2e99-4b93-ae82-7f4ab907aad4' AS UUID)
```
