...
- Creates an `exampleZone` distribution zone where data will only be stored on nodes that have SSD attribute:
  ...
  {% hint style="info" %}
  The example above assumes that there are nodes that have the attribute set in [node filtering](../administrators-guide/storage/distribution-zones.md#node-filtering) example.
  {% endhint %}
