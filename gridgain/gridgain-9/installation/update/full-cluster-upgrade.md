...
Take a [snapshot](../../administrators-guide/snapshots/snapshots-and-recovery.md) of your data before stopping the cluster. The snapshot is your only supported recovery path if a binary upgrade leaves persistent data unreadable on the new version. Verify that the snapshot completed successfully before proceeding.
