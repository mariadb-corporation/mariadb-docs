...
When loading large amounts of data using the [Data Streamer](../developers-guide/data-streamer.md), default settings may cause timeout failures or suboptimal throughput under heavy load. This page describes tuning options to improve stability and performance.
...
|---|---|---|---|
| [leaseExpirationIntervalMillis](../administrators-guide/config/cluster-config.md#replication-configuration) | 5000 | 10000 | Duration before a partition lease is considered expired. Increasing this gives heavily loaded nodes more time to renew their partition leases, reducing unnecessary primary replica changes under heavy I/O. |
| [retryTimeoutMillis](../administrators-guide/config/node-config.md#raft-configuration) | 10000 | 30000 | Timeout for Raft operation retries to receive a successful response from a remote peer. Increasing this reduces the chance of Raft failures during prolonged streaming sessions. |
...
|---|---|---|
| [perPartitionParallelOperations](../developers-guide/data-streamer.md#configuring-data-streamer) | 1 | Number of parallel streaming operations per partition. Increasing this value can reduce latency and improve throughput on clusters with available CPU and network capacity. |
| [pageSize](../developers-guide/data-streamer.md#configuring-data-streamer) | 1000 | Number of records per streaming page. Larger values reduce the number of round trips, which can improve latency, but increase memory usage per operation. |
