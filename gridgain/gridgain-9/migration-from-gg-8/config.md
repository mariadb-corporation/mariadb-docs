...
- Start GridGain 9 nodes as described in the [Quick Start](../quick-start/getting-started-guide.md);
...
Fore more information about the storage profiles, see [Storage Profiles and Engines](../administrators-guide/storage/README.md) and [Distribution Zones](../administrators-guide/storage/distribution-zones.md).
...
Data eviction is performed on volatile storage based on [storage engine](../administrators-guide/storage/README.md) configuration.

#### Expiry Policies

Expiry policies are now configured on a per-table basis by creating a column with `timestamp` data type and specifying it with a `EXPIRE AT` command. See the [SQL DDL](../sql-reference/ddl.md) reference.
...
You can configure REST properties in [node configuration](../administrators-guide/config/node-config.md). For more information about REST API, see the [REST API](../developers-guide/rest/rest-api.md) documentation and the provided [OpenAPI](https://www.gridgain.com/sdk/gridgain9/latest/openapi.html) specification.
...
Transparent data encryption is configured for the whole cluster at once, instead of being done on a per-cache basis. For information on configuring TDE, see [Transparent Data Encryption](../administrators-guide/security/tde.md) section.
...
In the current release, only `log` sink are supported. You can configure events as described in the [Events](../developers-guide/events/overview.md) section.
...
All security configuration is part of cluster configuration and is shared across all nodes. For more information on configuring security, see [Authentication](../administrators-guide/security/authentication.md) documentation.
