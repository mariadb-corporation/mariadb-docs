...
The `gridgainConfig` field controls per-node settings. The content uses the HOCON format that GridGain 9 expects for its node configuration files. For a full list of available node configuration parameters, see [Node Configuration Parameters](../../administrators-guide/config/node-config.md).
