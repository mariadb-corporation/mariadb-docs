...
By [default](../../administrators-guide/config/node-config.md), rest connector starts on port 10300. THis port can be configured in the `ignite.rest` [node configuration](../../administrators-guide/config/node-config.md#rest-configuration).
...
You are not limited to only monitoring, as GridGain REST API provides endpoints that can be used to manage the cluster as well. For example, you can create a [snapshot](../../administrators-guide/snapshots/snapshots-and-recovery.md) via REST:
...
On success, the endpoint returns the updated `ClusterTag` as JSON, including the new name and the unchanged cluster ID. The request fails with `400` if the supplied name is empty. The same operation is available from the CLI tool as [`cluster rename`](../../ignite-cli-tool.md#cluster-rename).
...
For the full list, see [User Permissions and Roles](../../administrators-guide/security/permissions.md).
...
Both limits are configurable, as described in [REST Configuration](../../administrators-guide/config/node-config.md#rest-configuration).
...
{% hint style="warning" %}
`TIMESTAMP` names an instant here, while the SQL type of that name is `DATETIME`. See [Data Types](../../sql-reference/data-types.md) for the full correspondence between SQL types and column types.
{% endhint %}
