---
description: >-
  Use standard ADO.NET components to work with GridGain SQL, including
  connecting, executing commands, reading data, parameters, and transactions.
---

# ADO.NET Integration

GridGain implements [ADO.NET](https://learn.microsoft.com/en-us/dotnet/framework/data/adonet/ado-net-overview) classes, such as `DbConnection`, `DbCommand`, `DbDataReader`, etc.,
...
To connect to a GridGain cluster, create a new connection with the connection string:
...
| HeartbeatInterval | Time span between heartbeat messages to keep connection alive in `hh:mm:ss.f` format. 1 second by default. |
...
{% hint style="info" %}
GridGain only supports input parameters. Parameter types are automatically inferred from the SQL query context, so you do not need to specify the parameter type explicitly.
{% endhint %}

GridGain supports parameterized queries using positional parameters. You can use the `IgniteDbConnection.CreateParameter()` method to create parameters that will replace the `?` placeholders in your query text.
...
{% hint style="info" %}
GridGain does not support custom isolation levels. All transactions are effectively `Serializable`.
{% endhint %}
...
The example below shows how you can work with a GridGain cluster via ADO:
