...
- `allowFollowerReads` - Whether the SQL engine can read data from non-primary replicas when executing this statement. Set `true` or `false` to override the cluster-wide [`sql.allowFollowerReads`](../../administrators-guide/config/cluster-config.md#sql-configuration) setting for this statement. Default value: `null` (the cluster-wide setting applies).
