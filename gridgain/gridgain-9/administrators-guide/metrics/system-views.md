---
description: >-
  Reference of the built-in SQL system views in the GridGain 9 SYSTEM schema,
  with the columns and data types of each view.
---
...
| PARTITION_STATE | STRING | Current state of the partition. Possible values:<br><br>• `AVAILABLE` – All replicas are healthy.<br>• `DEGRADED` – There are healthy replicas, and they form a majority.<br>• `READ_ONLY` – There are healthy replicas, but they don’t form a majority.<br>• `UNAVAILABLE` – There are no healthy replicas. |
...
| STATUS | STRING | **Deprecated**. Current status of the index. Possible values:<br><br>• `REGISTERED` - Index has been registered and is awaiting the start of building,<br>• `BUILDING` - Index is being built,<br>• `AVAILABLE` - Index is built and is ready to use,<br>• `STOPPING` - DROP INDEX command has been executed, index is waiting for running transactions to finish. |
| CATALOG_VERSION | INT32 | The version of the current catalog that contains this index. |
...
| CATALOG_VERSION | INT32 | The version of the current catalog that contains this index column. |
...
| LICENSE_EDITION | STRING | License edition. |
| LICENSE_COMMON_INFO | STRING | A list of key-value pairs providing information about the license requester/owner. |
| LICENSE_LIMITS | STRING | A list of key-value pairs providing information about the license validity period, cores and heap size limits, etc. |
| LICENSE_FEATURES | STRING | A list of GridGain features the license grants to the user. |
| ID | STRING | **Deprecated**. The license ID. |
| EDITION | STRING | **Deprecated**. License edition. |
| INFOS | STRING | **Deprecated**. A list of key-value pairs providing information about the license requester/owner. |
| LIMITS | STRING | **Deprecated**. A list of key-value pairs providing information about the license validity period, cores and heap size limits, etc. |
| FEATURES | STRING | **Deprecated**. A list of GridGain features the license grants to the user. |
...
| PARTITION_ID | INT32 | Identifier of the partition. |
| PARTITION_STATE | STRING | Current state of the local partition. Possible values:<br><br>• `UNAVAILABLE` – This state might be used when partition is not yet started, or it's already stopping, for example.<br>• `HEALTHY` – Alive partition with a healthy state machine.<br>• `INITIALIZING` – Partition is starting right now.<br>• `INSTALLING_SNAPSHOT` – Partition is installing a Raft snapshot from the leader.<br>• `CATCHING_UP` – Partition is catching up, meaning that it's not replicated part of the log yet.<br>• `BROKEN` – Partition is in broken state, usually it means that its state machine thrown an exception. |
...
| LOCK_MODE | STRING | The [lock mode](https://cwiki.apache.org/confluence/pages/viewpage.action?pageId=211885498#IEP91:Transactionprotocol-Lockingmodel). Possible values are:<br><br>• IS - intention shared lock<br>• S- shared lock<br>• IX - intention exclusive lock<br>• SIX - shared intention exclusive lock<br>• X - exclusive lock |
| TX_ID | STRING | **Deprecated**. The ID of the transaction that created the lock. |
| MODE | STRING | **Deprecated**. The [lock mode](https://cwiki.apache.org/confluence/pages/viewpage.action?pageId=211885498#IEP91:Transactionprotocol-Lockingmodel). Possible values are:<br><br>• IS - intention shared lock<br>• S- shared lock<br>• IX - intention exclusive lock<br>• SIX - shared intention exclusive lock<br>• X - exclusive lock |
...
| POLICY_ID | INT32 | Unique policy identifier. |
| ROLES | STRING | The comma-separated list of roles the policy applies to. |
| CONDITION | STRING | A boolean SQL expression over the columns of the target table. |
| CATALOG_VERSION | INT32 | The version of the current catalog that contains this policy. |
...
| SCHEMA_ID | INT32 | Unique schema identifier. |
| SCHEMA_NAME | STRING | The name of the schema. |
| CATALOG_VERSION | INT32 | The version of the current catalog that contains this schema. |
...
| CATALOG_VERSION | INT32 | The version of the current catalog that contains this sequence. |
...
| QUERY_PHASE | STRING | The query phase:<br><br>• INITIALIZATION - query registration and parsing<br>• OPTIMIZATION - query validation and plan optimization<br>• EXECUTION - query plan execution |
...
| SQL | STRING | The SQL query's expression. Password literals in `CREATE USER` and `ALTER USER` statements are replaced with `'******'`, including statements inside a multi-statement script. |
...
| PHASE | STRING | **Deprecated**. The query phase:<br><br>• INITIALIZATION - query registration and parsing<br>• OPTIMIZATION - query validation and plan optimization<br>• EXECUTION - query plan execution |
...
| QUERY_RETRY_COUNT | INT32 | The number of times the query was retried. 0 if the query was not retried. |
| QUERY_RETRY_REASON | STRING | The error message that triggered the most recent retry. NULL if the query was not retried. |
...
| VIEW_TYPE | STRING | System view type. Possible values:<br><br>• NODE - The view provides node-specific information. Data will be collected from all nodes, and represented in the view.<br>• CLUSTER - The view provides cluster-wide information. Data will be collected from one node, chosen to represent the cluster. |
...
| TYPE | STRING | **Deprecated**. System view type. Possible values:<br><br>• NODE - The view provides node-specific information. Data will be collected from all nodes, and represented in the view.<br>• CLUSTER - The view provides cluster-wide information. Data will be collected from one node, chosen to represent the cluster. |
| CATALOG_VERSION | INT32 | The version of the current catalog that contains this system view. |
...
| VIEW_NAME | STRING | System view name. |
| COLUMN_NAME | STRING | Column name. |
| COLUMN_TYPE | STRING | Column type. Can be any of the [supported types](../../sql-reference/data-types.md). |
...
| TYPE | STRING | **Deprecated**. Column type. Can be any of the [supported types](../../sql-reference/data-types.md). |
...
| CATALOG_VERSION | INT32 | The version of the current catalog that contains this system view column. |
...
| IS_CACHE | BOOLEAN | Defines if it is a [cache](../../developers-guide/cache.md). |
...
| SECONDARY_STORAGE_STATE | STRING | The current synchronization state of secondary storage. Possible values:<br><br>• `NOT_AVAILABLE` - The table does not have secondary storage configured.<br>• `SYNCING` - Secondary storage is being synchronized with primary storage.<br>• `AVAILABLE` - Secondary storage is synchronized and available for queries. |
...
| EXPIRE_COLUMN_ID | INT32 | The identifier of the column used for row [expiration](../../developers-guide/expiry-policies.md). Returns `NULL` if the table has no expiration column. |
| EXPIRE_COLUMN_NAME | STRING | The name of the column used for row [expiration](../../developers-guide/expiry-policies.md). Returns `NULL` if the table has no expiration column. |
| ARCHIVE_COLUMN_ID | INT32 | The identifier of the column used for [archiving](../archiving.md#data-archiving). Returns `NULL` if the table has no archive column. |
...
| CATALOG_VERSION | INT32 | The version of the current catalog that contains this table column. |
| DEFAULT_VALUE | STRING | The SQL representation of the column's default value, or `NULL` if default is `NULL` or is not set. Literals are shown in SQL form (for example, `'hello'`, `42`, `DATE '2020-01-01'`, `x'AABBCCDDEEFF'`). Function-based defaults are shown as their call form (for example, `RAND_UUID`, `NEXTVAL('SEQ_NAME')`, `CURRENT_TIMESTAMP + INTERVAL '7' DAYS`). |
...
| CATALOG_VERSION | INT32 | The version of the current catalog that contains this distribution zone. |
