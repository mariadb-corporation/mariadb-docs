...
[node configuration](../config/node-config.md), which must also enable file
...
| Name | Has Selector | Description |
|---|---|---|
|EXECUTE_SQL|no|Allows the user to execute SQL statements, including over the [REST API](../../developers-guide/rest/rest-api.md#running-sql). The user also needs the object privileges that a statement requires, such as `SELECT_FROM_TABLE`.|
...
A refused assignment or revocation is recorded as a [`USER_OPERATION_REFUSED`](../../developers-guide/events/events-list.md) event.
...
and the grant covers both existing and future tables in that schema. For more details check the `role privilege grant` [command syntax](../../ignite-cli-tool.md).
{% endhint %}
