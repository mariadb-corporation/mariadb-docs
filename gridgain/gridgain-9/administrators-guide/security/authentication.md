...
{% hint style="info" %}
This command writes to the `ignite.security` subtree, so it requires both the `WRITE_CLUSTER_CONFIG` and `WRITE_SECURITY_CONFIG` privileges. The same applies to every update under `ignite.security`, including the password change below. See [User Permissions and Roles](permissions.md).
{% endhint %}
