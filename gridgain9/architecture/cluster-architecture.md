---
description: >-
  A deeper look at the GridGain 9 system architecture: node components, cluster
  coordination, RAFT replication, transaction processing, and query execution.
---

# Cluster Architecture

GridGain 9 is a distributed database built on a modular component architecture. Each node runs the same set of services, enabling any node to handle client requests and participate in data storage. This topic provides a deeper look at how the pieces fit together — the node components, how the cluster coordinates itself, and how a request flows through the system.
...
flowchart TB
    subgraph "Client Layer"
        TC[Thin Clients<br/>Java, .NET, C++, Python]
        JDBC[JDBC/ODBC]
        REST[REST API]
    end
...
Every GridGain node runs the same component set. There is no distinction between "server" and "coordinator" nodes at the software level. Node roles emerge from RAFT group membership and placement driver lease assignments.
...
CMG membership is established during cluster initialization. Typically 3 to 5 nodes are selected as voting members; remaining nodes participate as learners. The CMG validates new nodes before admitting them to the cluster. See [Cluster Lifecycle](cluster-lifecycle.md) for how initialization and topology changes work.
...
- **Renewal**: Automatic, every half of the expiration interval
...
GridGain 9 uses multiple RAFT groups for different purposes. See [RAFT Consensus](raft-consensus.md) for a full explanation of the algorithm.
...
For how partitions are assigned and distributed, see [Data Partitioning](storage/data-partitioning.md) and [Distribution Zones](storage/distribution-zones.md).
...
GridGain 9 uses a hybrid concurrency control model:
...
Read-write transactions acquire locks through the primary replica. Read-only transactions use timestamp-based reads against MVCC version chains without acquiring locks. See [Transactions](transactions.md) for the full transaction model.
...
Exchange operators (Sender/Receiver) handle data movement between fragments. Colocation optimization reduces exchanges when joined data resides on the same nodes — see [Data Colocation](data-colocation.md).
...
Components start in dependency order, so that each component's dependencies are available before it starts:
...
- [Storage](storage/README.md) for storage-layer details and storage engines
- [Data Partitioning](storage/data-partitioning.md) for partition distribution and replication
- [Transactions](transactions.md) for transaction processing and MVCC
- [Data Consistency and Replication](data-consistency-and-replication.md) for the consistency model
