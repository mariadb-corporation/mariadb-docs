...
For the complete property reference, including acceptable value ranges, see the [Garbage Collection Configuration](../../reference/configuration/cluster-configuration-parameters.md#garbage-collection-configuration) section.
...
Low watermark configuration trades storage usage against how far into the past reads and recovery can reach. A larger `dataAvailabilityTimeMillis` keeps obsolete versions, deleted rows, and dropped tables on disk longer. As a result, read-only and time-travel queries, continuous queries, and [point-in-time recovery](../../gridgain9-management/snapshots/point-in-time-recovery.md) can all reach further back, but the cluster requires more storage. A smaller value reclaims storage sooner at the cost of a shorter read horizon.
...
TO keep track of LWM advancing, use the `mvgc.LastVacuumedLwm` metric. If you notice that the watermark has stalled, you can force the stale locks to be released with the [recovery low-watermark drop-locks](../../reference/cli-tool.md#recovery-low-watermark-drop-locks) CLI command. The command removes low watermark locks older than a given age on all cluster nodes:
