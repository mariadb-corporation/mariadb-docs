...
{% hint style="info" %}
If the continuous query consumer is slow, it is possible that the query will eventually fail, as it tries to read data that was already cleaned up due to falling behind the [low watermark](../architecture/storage/data-partitioning.md#version-storage).
{% endhint %}
