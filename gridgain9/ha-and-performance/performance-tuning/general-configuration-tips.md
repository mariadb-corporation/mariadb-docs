...
You can also change these properties for [distribution zones](../../reference/sql/distribution-zones.md) you have created for yourself.
...
GridGain stores data in off-heap memory, reserved individually for each [storage engine](../../architecture/storage/engines/native-persistent-storage.md) as required. However, Java Heap memory is still used to handle intermediary objects generated by workloads. For example:

- Cluster and node metadata. This includes information about what nodes are part of the cluster, internal logs, table [version chain](../../architecture/storage/data-partitioning.md#version-storage), what keys are locked for transactions and all other information required for GridGain to operate normally.
...
- [Compute](../../gridgain9-usage/distributed-computing/about-distributed-computing.md) operations are likely to use heap memory to store data. Specific requirements for compute jobs vary depending on what job is being performed.
