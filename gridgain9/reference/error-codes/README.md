...
| `IGN-META-7` | Failed to start a node because metastorage has diverged as a result of [MG recovery](../../gridgain9-management/disaster-recovery/system-groups-recovery.md#metastorage-group). | Clear the node of data and add it to the cluster as a blank node. |
...
| `IGN-REP-9` | Too many user operations are simultaneously waiting for a system replication group to become available, or a replication group is otherwise overloaded. | Reduce load or increase resources for the replication group. The limit on user operations waiting for a system group is controlled by [`replication.maxConcurrentWaitingUserOperations`](../configuration/cluster-configuration-parameters.md#replication-configuration). |
| `IGN-REP-10` | A user operation could not reach a quorum of a system replication group (the Cluster Management Group or Meta Storage) within the configured timeout, typically because of quorum loss or a missing leader. | Make sure a majority of the system group's nodes are online and that a leader has been elected, then retry the operation once the cluster recovers. The wait time is controlled by [`replication.longOperationTimeoutMillis`](../configuration/cluster-configuration-parameters.md#replication-configuration). |
| `IGN-REP-12` | Node rejected the request: accepting it would exceed the heap share reserved for pending partition operations. | Wait briefly and retry. If this error keeps appearing, the node is overloaded. Reduce client load, increase the node's JVM heap, or raise the value of [`replication.partitionOperationHeapUsagePercent`](../configuration/cluster-configuration-parameters.md#replication-configuration). |
...
| `GG-SNAPSHOTS-8` | Error indicating that an invalid snapshot path name has been used in the configuration for creation or restoration. This occurs when specifying a non-existent path as the destination or source for a snapshot. | Verify and correct the snapshot path name according to the [GridGain snapshot documentation](../../gridgain9-management/snapshots/data-snapshots.md#snapshot-location). Ensure the path exists in the configuration. |
...
| `GG-SNAPSHOTS-10` | During restoration of a snapshot, one of the target table partitions did not have a primary replica. This means that either a long ongoing rebalance was under way or one of the target tables belonged to a [distribution zone](../../architecture/storage/distribution-zones.md) without data nodes. | Retry the request and, if the issue persists, check that the target distribution zones contain at least one data node. |
...
| `GG-SNAPSHOTS-12` | The partition layout in the destination [distribution zone](../../architecture/storage/distribution-zones.md) does not match the snapshot's layout. Either the source and destination zones have different partition counts, or, for a local snapshot, the per-node distribution no longer covers every partition assigned to this node. | Recreate the destination zone with the same number of partitions as the snapshot. For local snapshots, also ensure that the destination cluster topology preserves the original per-node partition distribution. |
