---
description: >-
  Reference for all relational operators that can appear in a GridGain 9 EXPLAIN
  plan, with their semantics and supported attributes.
---
...
- `collation`: Optional. List of columns and the expected sort order this operator relies on. Present only when the exchange preserves a sort order.
- `est`: Estimated number of output rows.
