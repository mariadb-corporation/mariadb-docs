...
To use REST authentication, first [enable](../../security/authentication.md) it on in cluster configuration.
...
Issued tokens expire according to cluster [security](../configuration/cluster-configuration-parameters.md#security-configuration) configuration. If a token needs to be revoked sooner, you can cancel it by sending a dedicated endpoint. GridGain supports two ways to revoke tokens:
...
{% hint style="info" %}
The user who attempts to revoke tokens must have the `REVOKE_TOKEN` [permission](../../security/user-permissions-and-roles.md) on the cluster. Otherwise, the operation will fail.
{% endhint %}
