...
{% hint style="info" %}
These metrics become available when at least one [replication](../../gridgain9-management/data-center-replication/configuring-replication.md) exists.
{% endhint %}
...
The metrics in this section track the progress of the [low watermark](../../architecture/storage/low-watermark-and-gc.md). Use them to check whether the low watermark is still advancing, and to find what is holding it back when it is not.
...
{% hint style="info" %}
Each [storage profile](../../architecture/storage/README.md) with `aipersist` storage engine has an individual metrics exporter.
{% endhint %}
